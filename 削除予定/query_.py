...
def get_pref_list() -> list[str]:
    """都道府県名の一覧を返す（順番はarea_code順）。"""
    sql = "SELECT pref FROM read_csv_auto(?) ORDER BY area_code"
    rows = duckdb.execute(sql, [MASTER_PREF_PATH]).fetchall()
